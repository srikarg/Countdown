...
var displayCountdowns = function () {
  chrome.storage.sync.get('countdowns', function (data) {
    ...
    if ($.isEmptyObject(data)) {
      newMessage(
        'Try adding some countdowns by clicking the button below!',
        'blue'
      )
    } else {
...
var createCountdown = function (id, title, end) {
  ...
  if (!$('#' + id).length) {
    countdowns.append(
      '<li id="' +
        id +
        '" class="u-full-width"><span class="close">&times;</span><h1 class="title"></h1><p class="countdown"></p></li>'
    )
  }
  ...
  $('.event-form').on('submit', function (e) {
    ...
    if ($('#enterTime').is(':checked')) {
      ...
      if (time) {
        ...
        newMessage(
          "Please enter a time! If you don't want a time, uncheck the time checkbox",
          'red'
        )
        return
      }
